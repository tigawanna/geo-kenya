...
const filter_collection = "^wards_"
export async function callCliDirectly() {
  ...
  const commands = ["npx", "@tigawanna/typed-pocketbase", "--email", email, "--password", password,"--type","ts,zod"];
  if (url && url.length > 0) {
  ...
  if (filter_collection) {
    commands.push("--include", filter_collection);
  }
